...
const RFQSchema = new Schema(
  {
    ...
    termsAndConditions: {
      ...
      validate: {
        ...
        message:
          "Terms and conditions must include at least one field and a timestamp",
      },
    },
  },
  {
...
RFQSchema.pre("save", function (next) {
  ...
  if (rfq.generalDetails) {
    ...
    rfq.generalDetails.fields = rfq.generalDetails.fields.map(
      (field: any): FieldMetadata => ({
        id: field.id || "",
        label: field.label || "",
    ...
    rfq.generalDetails.subsections = rfq.generalDetails.subsections.map(
      (subsection: any): Subsection => ({
        id: subsection.id || "",
